        ...
        function InitMap()
        {
            ...
            map.events.add('ready', function () {
                ...
                dataSource.add(new atlas.data.Feature(new atlas.data.Point(<?php echo json_encode ( $locationData[0]["coordinates"] ) ; ?>), {
                    deviceGuid: 'deviceGuid: <?php echo json_encode ( $locationData[0]["deviceGuid"] ) ;?>',
                    createdDate: 'createdDate: <?php echo json_encode ( $locationData[0]["createdDate"] ) ;?>',
                    receivedDate: 'receivedDate: <?php echo json_encode ( $locationData[0]["receivedDate"] ) ;?>'
                }));
                dataSource.add(new atlas.data.Feature(new atlas.data.Point(<?php echo json_encode ( $locationData[3]["coordinates"] ) ; ?>), {
                    deviceGuid: 'deviceGuid: <?php echo json_encode ( $locationData[3]["deviceGuid"] ) ;?>',
                    createdDate: 'createdDate: <?php echo json_encode ( $locationData[3]["createdDate"] ) ;?>',
                    receivedDate: 'receivedDate: <?php echo json_encode ( $locationData[3]["receivedDate"] ) ;?>'
                }));

                // To poniżej nie zadziała. Bo php nie wie co to i.
                // for ( let i = 0 ; i < <?php //echo count ( $locationData ) ; ?> ; i++ )
                // {
                //     dataSource.add(new atlas.data.Feature(new atlas.data.Point(<?php //echo json_encode ( $locationData[i]["coordinates"] ) ; ?>), {
                //         deviceGuid: 'deviceGuid: <?php //echo json_encode ( $locationData[i]["deviceGuid"] ) ;?>',
                //         createdDate: 'createdDate: <?php //echo json_encode ( $locationData[i]["createdDate"] ) ;?>',
                //         receivedDate: 'receivedDate: <?php //echo json_encode ( $locationData[i]["receivedDate"] ) ;?>'
                //     }));
                // }
